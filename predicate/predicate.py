...
class XorPredicate[T](Predicate[T]):
    ...
    @property
    def always_false(self) -> bool:
        return (self.predicate_1.always_false and self.predicate_2.always_false) or (self.predicate_1.always_true and self.predicate_2.always_true)

    @property
    def always_true(self) -> bool:
        return (self.predicate_1.always_true and self.predicate_2.always_false) or (self.predicate_1.always_false and self.predicate_2.always_true)


def to_filtered(l: list[str | None], predicate: Predicate[str | None]) -> list[str | None]:
...
def gt_p[T](v: T) -> Predicate[T]:
    def _gt(x: T) -> bool:
        return x > v
    return Predicate(_gt)


def le_p[T](v: T) -> Predicate[T]:
    return Predicate(lambda x: x <= v)


def lt_p[T](v: T) -> Predicate[T]:
    return Predicate(lambda x: x < v)


def any_p[T](predicate: Predicate[T]) -> Predicate[Iterable[T]]:
    return Predicate(lambda l: any(predicate(x) for x in l))


def all_p[T](predicate: Predicate[T]) -> Predicate[Iterable[T]]:
    return Predicate(lambda l: all(predicate(x) for x in l))
...
@dataclass
class AlwaysTruePredicate[T](Predicate[T]):
    def __init__(self):
...
@dataclass
class AlwaysFalsePredicate[T](Predicate[T]):
    def __init__(self):
